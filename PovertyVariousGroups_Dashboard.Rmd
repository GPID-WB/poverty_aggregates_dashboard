...
output$dynamic_sidebar <- renderUI({
  ...
  else if (tab == "Compare Groups (Overlay)") {
  tagList(
    # Selection A
    selectInput("group_var1",
                "Select grouping for Line A",
                choices = c("Region (new WB classification)",
                            "Region (old PovcalNet classification)",
                            "Income group (historical)",
                            "Income group (latest)",
...
filtered_data <- reactive({
  ...
  if (tab == "Compare Groups (Overlay)") {
    ...
    dB <- dt[
      ...
      line_label = "Line B"     # ← and here
    )]


    overlay <- data.table::rbindlist(list(dA, dB), use.names = TRUE, fill = TRUE)
...
output$download_filtered <- downloadHandler(
  ...
  content = function(file) {
    ...
    data <- ov[, .(year, group, definition, headcount)]
  }
...
output$download_filtered_dta <- downloadHandler(
  ...
  content = function(file) {
    ...
    data <- filtered_data()$graph_overlay[, .(year, group, definition, headcount)]
    }
...
tabsetPanel(
    ...
    HTML("
      <div style='margin-bottom: 15px; font-size: 14px; line-height: 1.6; max-width: 100%;'>
        ...
        <p>
          ...
          <strong>Sub-Saharan Africa</strong> and <strong>Latin America & Caribbean</strong> are the only two regions whose composition remains unchanged. This dashboard allows users 
  ...
  tabPanel("Compare Groups (Side-by-Side)"),
  tabPanel("Compare Groups (Overlay)")
)
...
output$main_plot <- renderPlot({
  ...
  if (input$active_tab == "Poverty Headcount and the Number of Poor") {
    ...
    # ---- NEW: Compare Groups (Overlay) ----
  else if (input$active_tab == "Compare Groups (Overlay)") {
    ...
    p <- ggplot(
      ...
      geom_line(linewidth = 1) +
      labs(
        title = "Poverty rate: Compare Groups (Overlay)",
        y     = "Poverty headcount, %",
        x     = ""
      ) +
      my_theme(by = input$group_var1) +
