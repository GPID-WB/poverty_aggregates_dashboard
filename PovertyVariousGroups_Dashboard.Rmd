    ...
    tagList(
      selectInput("group_var1", "Select the group to chart",
                  choices = c("Region (new WB classification)",
                              ...
                              "Region (new WB classification, excluding HICs)",
...
filtered_data <- reactive({
  ...
  d1 <- dt[
    ...
    d1[cov != 1, headcount := NA_real_]
    
    d0 <- merge(d0, cov_flags, by = c(group_var, "year"), all.x = TRUE)
    d0[cov != 1, pop_in_pov := NA_real_]
...
if (tab == "Compare groups") {
  ...
  # --- coverage gaps for overlay (no dropping rows)
    ...
    dA <- merge(dA, cov1, by.x = c("group", "year"),
                     ...
                     all.x = TRUE)
    dA[cov != 1, `:=`(headcount = NA_real_, pop_in_pov = NA_real_)]
    
    dB <- merge(dB, cov2, by.x = c("group", "year"),
                     by.y = c(underscore_var(input$group_var2), "year"),
                     all.x = TRUE)
    dB[cov != 1, `:=`(headcount = NA_real_, pop_in_pov = NA_real_)]
