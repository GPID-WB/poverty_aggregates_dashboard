...
output$download_filtered_csv <- downloadHandler(
  ...
  data <- if (tab == "Poverty headcount and the number of poor") {
    d1 <- filtered_data()$graph1   # year, gv1, headcount (0-1)
    d0 <- filtered_data()$graph0   # year, gv1, pop_in_pov
    gv1 <- underscore_var(input$group_var1)

    merge(d1, d0, by = c("year", gv1), all = TRUE)[
      , .(
        year,
  ...
  data <- ov[
    , .(
      ...
      poverty_line         = pl,
      headcount            = headcount,    # keep scale as-is (0–1)
      pop_in_poverty       = round(pop_in_pov)
    )
  ]
}
