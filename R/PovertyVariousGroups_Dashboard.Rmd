...
filtered_data <- reactive({
  ...
  d0 <- dt[
    get(input$group_var1) %in% sel_g1,
    .(pop_in_pov = sum(pop_in_pov, na.rm = TRUE)),
    by = c("year", input$group_var1)
  ]
  ...
  if (tab == "Comparison") {
    ...
    if (is.null(sel_g2)) sel_g2 <- unique(dt[[input$group_var2]])
    d2 <- dt[
      get(input$group_var2) %in% sel_g2,
      .(headcount = weighted.mean(headcount, pop, na.rm = TRUE)),
      by = c("year", input$group_var2)
    ]
  } else {
    d2 <- data.table()
  }
